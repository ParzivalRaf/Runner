...
public static class RunnerLookBuilder
{
    ...
    // Туман должен закрывать даль ДО того места, где появляются новые чанки,
    // иначе видно, как декорации возникают из воздуха. ChunkSpawner держит
    // 4 чанка впереди по 30 юнитов = 120.
    //
    // Было 70/185 — на отметке 120 туман набирал всего 43 %, то есть своей
    // работы не делал: дальние дома оставались насыщенными, а появление
    // чанков было видно. 55/160 даёт на 120 метрах 59 %, а на дальнем
    // слое фона (128) — 66 %. Это и есть эффект «даль тонет в дымке»:
    // на референсе дальний город бледный, почти цвета неба, но различимый.
    // Уводить FogEnd ниже 140 не стоит — горизонт станет пустым небом.
    private const float FogStart = 55f;
    private const float FogEnd = 160f;
    ...
    private static void SetUpSun()
    {
        ...
        // и картинка плоская.
        // 48° давало короткие тени: предметы стояли почти в собственных
        // пятнах. 30° — солнце низкое, тени вытянуты в полтора корпуса
        // и ложатся поперёк дороги. На референсе именно они и создают
        // половину глубины кадра. Ниже 25° не опускать: тени начнут
        // отрываться от подошв (артефакт карты теней на косом свете).
        sun.transform.rotation = Quaternion.Euler(30f, 214f, 0f);
        ...
        // 0.85: на референсе тень заметно темнее полутени. Чёрной она
        // не станет — остаток засвечивает рассеянный свет неба.
